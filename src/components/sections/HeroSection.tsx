...
export function HeroSection() {
  ...
  const heroContent = (
    <>
      <p
        className="hero-script-optical block w-max mx-auto text-[var(--text-primary)] text-3xl sm:text-4xl md:text-5xl mb-1"
        style={{ fontFamily: "var(--font-script)" }}
      >
        {hero.saveTheDate}
      </p>
      <h1
        className="hero-tracking-compensate flex flex-col items-center gap-0 font-serif uppercase text-[var(--text-primary)] mt-4 sm:mt-5 md:mt-6 leading-tight tracking-[0.2em] sm:tracking-[0.28em] md:tracking-[0.32em] text-4xl sm:text-5xl md:text-6xl lg:text-7xl w-max mx-auto"
        style={{ fontFamily: "var(--font-serif)" }}
        aria-label={`${couple.name1} y ${couple.name2}`}
      >
        <span className="block w-max mx-auto">{couple.name1}</span>
        <span className="block w-max mx-auto text-3xl sm:text-4xl md:text-5xl lg:text-6xl">&</span>
        <span className="block w-max mx-auto">{couple.name2}</span>
      </h1>
      <p
        className="hero-script-optical block w-max mx-auto text-[var(--text-primary)] text-2xl sm:text-3xl md:text-4xl mt-5 sm:mt-6 md:mt-8"
        style={{ fontFamily: "var(--font-script)" }}
      >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <motion.span variants={variants} className="block w-max mx-auto">{couple.name1}</motion.span>
          <motion.span variants={variants} className="block w-max mx-auto text-3xl sm:text-4xl md:text-5xl lg:text-6xl">&</motion.span>
          <motion.span variants={variants} className="block w-max mx-auto">{couple.name2}</motion.span>
        </motion.h1>
